...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Subtitle
    heroTl.from('[data-animate="subtitle"]', {
        y: 30,
        opacity: 0,
        duration: 0.8
    }, '-=0.5');
    ...
    // Scroll indicator
    heroTl.from('[data-animate="scroll"]', {
        opacity: 0,
        y: 20,
        duration: 0.6
    }, '-=0.2');
    
    // Floating previews parallax
    gsap.utils.toArray('.wc-floater').forEach((floater, i) => {
        gsap.to(floater, {
            y: (i + 1) * -50,
            rotation: (i - 1) * 5,
            scrollTrigger: {
                trigger: '.wc-portfolio-hero',
                start: 'top top',
                end: 'bottom top',
                scrub: 1
            }
    ...
    // ========================================
    // HERO PARALLAX SCROLL EFFECTS - subtle only
    // ========================================
    gsap.to('.wc-portfolio-hero__content', {
        scrollTrigger: {
            trigger: '.wc-portfolio-hero',
            start: 'top top',
            end: '50% top',
        ...
        y: -40
        // Removed opacity: 0 - content should stay visible
    });
    
    // ========================================
    // PORTFOLIO TEASER SECTION ANIMATIONS
    // ========================================
    
    // Intro section with elegant reveal - slower, smoother
    const teaserIntro = document.querySelector('.wc-teaser-intro');
    if (teaserIntro) {
        gsap.fromTo(teaserIntro, 
            { y: 40, opacity: 0 },
            {
                scrollTrigger: {
                    trigger: '.wc-portfolio-teaser-section',
                    start: 'top 85%',
    ...
    // Category headers and cards - slower, coordinated timing
    document.querySelectorAll('.wc-teaser-category').forEach((category, i) => {
        const header = category.querySelector('.wc-teaser-category__header');
        const cards = category.querySelectorAll('.wc-teaser-card');
        const delay = i * 0.15; // Slight delay between categories
    ...
    // ========================================
    // SCROLL-OUT PARALLAX EFFECTS - DISABLED
    // These were causing content to disappear permanently
    // ========================================
    
    // Tech section parallax fade out - gentler
    gsap.to('.wc-tech-header', {
        scrollTrigger: {
            trigger: '.wc-tech-header',
            start: 'top 15%',
            end: 'bottom -10%',
            scrub: 1.5
        },
    ...
    // CTA section - no fade out, keep visible
    ...
    // ========================================
    // PROJECT CARDS - 3D TILT + STAGGERED REVEAL
    // ========================================
    ...
    cards.forEach(card => {
        const inner = card.querySelector('.wc-project-card__inner');
        const shine = card.querySelector('.wc-project-card__shine');
